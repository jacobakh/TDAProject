...
btc = read.csv("btc.csv", header = F) #read in table
btc = btc[-1, ]    
btc$V1 = as.Date(btc$V1, "%m/%d/%Y")

  names(btc) <- as.character(unlist(btc[1,]))
  btc <- btc[-1,]

columnNames = colnames(btc)
dates <- row.names(btc) #extract date feature
dates_ <- as.data.frame(dates)
dates_$ID <- seq.int(nrow(dates_))
row.names(btc) <- seq.int(nrow(btc)) #reset row names from 1 to the number of rows in the dataset

test <- btc %>%
  select(date, txVolume.USD.)
btc = btc[1:8]
